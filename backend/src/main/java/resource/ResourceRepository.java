...
public class ResourceRepository implements PanacheMongoRepository<Resource> {
    ...
    public List<ResourceEmpty> findResourcesCanAssigned() {
        List<Resource> resources = findAll().list();
        List<ResourceEmpty> result = new ArrayList<>();
        Double working = 0.0;
        for (Resource resource : resources) {
            ResourceEmpty resourceEmpty = new ResourceEmpty();
            if (resource.getProjects() == null) {
                // result.add(resource);
                resourceEmpty.setPrefix(resource.getPrefix());
                resourceEmpty.setFirstName(resource.getFirstName());
                resourceEmpty.setLastName(resource.getLastName());
                resourceEmpty.setPosition(resource.getPosition());
                resourceEmpty.setWorking(0.0);
                result.add(resourceEmpty);

            } else {
                for (SubProject project : resource.getProjects()) {
                    WorkingDetail workingDetail = project.getWorkingDetail().get(project.getWorkingDetail().size() - 1);
                    // System.out.println(workingDetail.getWorking());
                    working += workingDetail.getWorking();
                }
                ...
                if (working < 100.0) {
                    ...
                    resourceEmpty.setLastName(resource.getLastName());
                    resourceEmpty.setPosition(resource.getPosition());
                    resourceEmpty.setWorking(working);
                    result.add(resourceEmpty);
                    working = 0.0;
                }
    ...
    public List<ChartModelResponse> getResourceInRange() {
        ...
        // List<ResourceDetailInChart> resourceDetailInChart = new
        // ArrayList<ResourceDetailInChart>();
        // ChartModelResponse chartModelResponse = new ChartModelResponse();
        // chartModelResponse.setMonth(i);
        // chartModelResponse.setResources(resourceDetailInChart);
        // chartModelResponses.add(chartModelResponse);
        ...
        for (Resource resource : resources) {
            ...
            for (SubProject subProject : resource.getProjects()) {
                ...
                LocalDate jodaDateEndDate = convertToLocalDate(
                        subProject.getWorkingDetail().get(subProject.getWorkingDetail().size() - 1).getEndDate());
                ...
                if (jodaDateEndDate.isBefore(d1)) {
                    ...
                    if (!historyEmpResource.contains(resource.getEmpNo())) {
                        ...
                        resourceDetailInChart.setEndDate(subProject.getWorkingDetail()
                                .get(subProject.getWorkingDetail().size() - 1).getEndDate());
                        int monthInEnd = subProject.getWorkingDetail().get(subProject.getWorkingDetail().size() - 1)
                                .getEndDate().getMonth();
